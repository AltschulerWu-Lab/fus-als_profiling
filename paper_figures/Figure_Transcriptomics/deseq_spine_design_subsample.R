...
#' in the most consistent p-values for spinal cord samples across data subsample 
#' replicates.
#' 
...
designs <- str_c('~ALS + ', designs)

################################################################################
#' # Stability analysis of DESeq designs
#' To evaluate different designs, we take subsamples of the full data set, 
#' compute deseq p-values over each subsample, and compare the correlation 
...
deseq.ss <- replicate(n.subsample, {
  subsample_deseq(x.spine[,genes], xmeta.spine, designs, name="ALSTRUE")
}, simplify=FALSE)
...
if (save.fig) pdf(file=file.path(output.fig.dir, 'spine_sability.pdf'), h=12, w=12)
